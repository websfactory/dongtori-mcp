...
DESC_DROP_IN = (
    "[동토리: 우리 동네 아이 프로그램] 예약이나 신청 없이 아이와 바로 방문할 수 있는 상시 개방 장소를 찾습니다. "
...
def _find_drop_in(sido, sigungu, ages, free_only, inclusive):
    ...
    shown = order[:TOP_N]
    cards = [render.dropin_venue_card(reg["sido"], groups[k], free_only) for k in shown]
    ...
    # 휴관 요일 ↔ 방문 시점 모순 방어: 호스트가 놓쳐도 사용자에게 닿는 서버 측 안전망
    # (호스트가 "이번 주말" 질문에 토·일 휴관 장소를 그대로 추천한 실측 사례. ★"주말" 단어 금지)
    if any(groups[k][0].get("closed_label") for k in shown):
        out += "\n\n_정기 휴관 요일이 있는 곳은 방문하실 날과 겹치는지 꼭 확인해 주세요._"
...
DESC_SIGN_UP = (
    "[동토리: 우리 동네 아이 프로그램] 미리 신청·접수해야 참여할 수 있는 아이 프로그램을 찾습니다. "
...
def _find_sign_up(sido, sigungu, ages, free_only, inclusive):
    ...
    shown = hits[:TOP_N]
    cards = [render.apply_card(reg["sido"], i, today, free_only) for i in shown]
...
DESC_PLACE = (
    "[동토리: 우리 동네 아이 프로그램] 특정 시설·장소에서 지금 하는 아이 프로그램과 행사를 한눈에 보여줍니다. "
...
def _find_place(place_name, sido, sigungu):
    ...
    for i in shown:
        ...
        if i.get("participation_type") == "apply":
            bits.append(render.fmt_deadline(i, today))
...
DESC_DETAIL = (
    "[동토리: 우리 동네 아이 프로그램] 앞서 찾은 장소·프로그램 하나의 자세한 정보를 가져옵니다: "
